...
//
status_t showTestFrame(ANativeWindow *w, const FRAME &f, bool isShow, int frameCnt, int delayMs) {
    ...
    for (int i = 0; i < frameCnt; i++) {
...

        {
            ...
            if (ptr != NULL) {
                ...
                //printf("data [%08x] [%08x] ", *(uint32_t*)ptr, *(uint32_t*)((uint32_t)ptr + len - 4));
            }
            gb->unlock();
        }
...
status_t showTestUiFrame(ANativeWindow *w, bool isShow, int frameCnt, int delayMs, int fw, int fh) {
    char value[PROPERTY_VALUE_MAX];
...
status_t main(/*int argc, char** argv*/) {
   ...
   int s1h = (disph -200) / 2;
    sp<SurfaceControl> surfaceControl1 = client->createSurface(
        surfaceName1, s1w, s1h, PIXEL_FORMAT_RGBA_8888);
    ...
    for(int i = 0; i < rnd; i++) {    
        showTestUiFrame(window1, true, 10, 16, s1w, s1h);
        /*
        SurfaceComposerClient::openGlobalTransaction();
        if (isShow1) {
    ...
    for(int i = 0; i < rnd; i++) {
        ...
        if (mode_s1>=0)
            showTestFrame(window1, test_frames[k], isShow1, frameCnt, delayMs);
        if (mode_s2>=0)
            showTestFrame(window2, test_frames[k], isShow2, frameCnt, delayMs);

        printf("done\n");
        if ((k + 1) == TEST_FRAMES) {
            SurfaceComposerClient::openGlobalTransaction();
            int session = (i / TEST_FRAMES) % 2;
            if (session == 0) {
                surfaceControl1->hide();
                isShow1 = false;
            } else {
